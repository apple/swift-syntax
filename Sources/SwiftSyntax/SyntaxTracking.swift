...
fileprivate extension Comparable {
  /// Adjsuts this value so that it guarenteed to be in `range`.
...
fileprivate extension Range<SyntaxIndexInTree> {
  func translated(by offset: Int) -> Range<SyntaxIndexInTree> {
    return SyntaxIndexInTree(UInt32(Int(self.lowerBound.indexInTree) + offset))..<SyntaxIndexInTree(UInt32(Int(self.upperBound.indexInTree) + offset))
  }
}
...
struct SyntaxTracking {
  ...
  func replacing(oldIndexInTree: SyntaxIndexInTree, oldTotalNodes: Int, by newNode: Syntax?) -> SyntaxTracking {
    ...
    if let newNode, let newNodeSyntaxTracking = newNode.syntaxTracking, newNodeSyntaxTracking.trackedTree == trackedTree {
  ...
  ///
  /// - Complexity: O(trackedRanges.count)
  func originalIndexInTree(of derivedIndexInTree: SyntaxIndexInTree) -> SyntaxIndexInTree? {
...
extension Syntax {
  ...
  fileprivate var tracked: Syntax {
    if let parent {
      ...
      let result = Syntax.forRoot(self.raw, rawNodeArena: self.raw.arena)
...
class IndexInTreeFinder: SyntaxAnyVisitor {
  ...
  init(indexInTree: SyntaxIndexInTree) {
...
extension SyntaxTracking {
  ...
  init?(tracking nodes: [Syntax?]) {
    ...
    var indexInTree = SyntaxIndexInTree.zero.advancedToFirstChild()
    for case .some(let node) in nodes {
  ...
  init?(trackingRoot node: Syntax) {
    ...
    self.init(trackedTree: treeToTrack, trackedRanges: [])
    self.track(originalNode: node, nodeIndexInNewTree: SyntaxIndexInTree.zero)
  }
}
