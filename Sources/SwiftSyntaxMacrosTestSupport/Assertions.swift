...
public struct NoteSpec {
  ...
  public init(
    ...
    line: Int,
    column: Int,
    originatorFileID: String = #fileID,
    originatorFilePath: StaticString = #filePath,
    ...
    self.line = line
    self.column = column
    self.originatorFileID = originatorFileID
    self.originatorFilePath = originatorFilePath
    self.originatorLine = originatorLine
    self.originatorColumn = originatorColumn
  }
}
...
func assertNote(
  ...
  failingWith failureHandler: ((_ message: String, _ fileID: String, _ filePath: StaticString, _ line: UInt, _ column: UInt) -> Void)? = nil
) {
  let failureHandler = failureHandler ?? defaultFailureHandler
  ...
  if location.column != spec.column {
    failureHandler(
      "column \(location.column) of note does not match \(spec.column)",
      spec.originatorFileID,
      spec.originatorFilePath,
      spec.originatorLine,
      spec.originatorColumn
...
func assertFixIt(
  ...
  let failureHandler = failureHandler ?? defaultFailureHandler
  if fixIt.message.message != spec.message {
    let message = describeDifferenceBetweenStrings(fixIt.message.message, spec.message, "message of Fix-It does not match")
    failureHandler(
      message,
      spec.originatorFileID,
      spec.originatorFilePath,
...
extension DiagnosticSpec {
  ...
  @_disfavoredOverload
  @available(*, deprecated, message: "Use init(id:message:line:column:severity:highlights:notes:fixIts:originatorFileID:originatorFilePath:originatorLine:originatorColumn:) instead")
  public init(
    id: MessageID? = nil,
    message: String,
    line: Int,
...
func assertDiagnostic(
  ...
  if let id = spec.id, diag.diagnosticID != id {
    failureHandler(
      "diagnostic ID \(diag.diagnosticID) does not match \(id)",
      spec.originatorFileID,
      spec.originatorFilePath,
      spec.originatorLine,
      spec.originatorColumn
  ...
  if diag.message != spec.message {
    let message = describeDifferenceBetweenStrings(diag.message, spec.message, "message does not match")
    failureHandler(
      message,
      spec.originatorFileID,
      spec.originatorFilePath,
  ...
  if location.line != spec.line {
    failureHandler(
      "line \(location.line) does not match \(spec.line)",
      spec.originatorFileID,
      spec.originatorFilePath,
      spec.originatorLine,
      spec.originatorColumn
  ...
  if location.column != spec.column {
    failureHandler(
      "column \(location.column) does not match \(spec.column)",
      spec.originatorFileID,
      spec.originatorFilePath,
      spec.originatorLine,
      spec.originatorColumn
  ...
  if spec.severity != diag.diagMessage.severity {
    failureHandler(
      "severity \(spec.severity) does not match \(diag.diagMessage.severity)",
      spec.originatorFileID,
      spec.originatorFilePath,
      spec.originatorLine,
      spec.originatorColumn
  ...
  if let highlights = spec.highlights {
    if diag.highlights.count != highlights.count {
      ...
      for (actual, expected) in zip(diag.highlights.lazy.map(\.trimmedDescription), highlights) {
        if actual != expected {
          let message = describeDifferenceBetweenStrings(actual, expected, "highlight does not match")
          failureHandler(
            message,
            spec.originatorFileID,
            spec.originatorFilePath,
  ...
  if diag.notes.count != spec.notes.count {
    ...
    for (note, expectedNote) in zip(diag.notes, spec.notes) {
      assertNote(note, in: expansionContext, expected: expectedNote, failingWith: failureHandler)
    }
  }
  if diag.fixIts.count != spec.fixIts.count {
    ...
    for (fixIt, expectedFixIt) in zip(diag.fixIts, spec.fixIts) {
      assertFixIt(fixIt, expected: expectedFixIt, failingWith: failureHandler)
    }
  }
}
...
public func assertMacroExpansion(
  ...
  do {
    let expandedSourceFile = expandedSourceFile.description.trimmingCharacters(in: .newlines)
    let expectedExpandedSource = expectedExpandedSource.trimmingCharacters(in: .newlines)
    if expandedSourceFile != expectedExpandedSource {
      let message = describeDifferenceBetweenStrings(
        expandedSourceFile,
      ...
      failureHandler(message, fileID, filePath, line, column)
    }
  }
  ...
  if context.diagnostics.count != diagnostics.count {
    ...
    for (actualDiag, expectedDiag) in zip(context.diagnostics, diagnostics) {
      assertDiagnostic(actualDiag, in: context, expected: expectedDiag, failingWith: failureHandler)
    }
  }
  ...
  if let expectedFixedSource = expectedFixedSource {
    ...
    do {
      let fixedTreeDescription = fixedTreeDescription.trimmingTrailingWhitespace()
      let expectedFixedSource = expectedFixedSource.trimmingTrailingWhitespace()
      if fixedTreeDescription != expectedFixedSource {
        let message = describeDifferenceBetweenStrings(fixedTreeDescription, expectedFixedSource)
        failureHandler(message, fileID, filePath, line, column)
      }
    }
  }
