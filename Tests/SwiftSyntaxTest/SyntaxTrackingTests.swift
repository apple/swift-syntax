...
public class SyntaxTrackingTests: XCTestCase {
  func testReuseSubtreeWithoutNodeShiftInSameArena() {
    ...
    XCTAssertEqual(newAccessorBlock.accessors.originalNode(in: originalAccessorBlock)?.id, originalAccessorBlock.accessors.id)
  }
  ...
  func testReplaceOtherNodeWithoutNodeShift() {
    ...
    XCTAssertEqual(newAccessor.accessors.originalNode(in: originalAccessorBlock)?.id, originalAccessorBlock.accessors.id)
  }
